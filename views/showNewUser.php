                        ...
                        echo "<thead><tr><th>Id</th><th>Nombre</th><th>Apellidos</th><th>Email</th><th>Nick</th><th>Contraseña</th><th>Tipo</th><th>Accion1</th><th>Accion2</th></tr></thead>";
                            ...
                            while($muestraTodosUsuarios = $buscaTodosUsuarios->fetch_object()){
                               echo "<tr><td>".$muestraTodosUsuarios->id." <td>".$muestraTodosUsuarios->nombre."<td> ".$muestraTodosUsuarios->apellidos."<td> ".$muestraTodosUsuarios->email." <td>".$muestraTodosUsuarios->nick."<td> ".$muestraTodosUsuarios->password." <td>".$muestraTodosUsuarios->tipo;
                                ?>
                                <td><input type="button" class="btn btn-link" value="Modificar" name="modificar" OnClick="location.href='index.php?do=editUser&id=<?php echo $muestraTodosUsuarios->id? ">
                                <td><input type="button" class="btn btn-link" value="Eliminar" name="eliminar" OnClick="location.href='index.php?do=deleteUser&id=<?php echo $muestraTodosUsuarios->id? ">
                                </td>
                               </tr>  
                            <?php  
                            }
                            ...
                            <input type="button" class="btn btn-primary" value="Nuevo Usuario" name="nuevo" OnClick="location.href='index.php?do=showFormNewUser'">
